...
CREATE TABLE sys_user (
    ...
    wx_openid VARCHAR(64) DEFAULT NULL COMMENT '微信小程序 openid（微信一键登录），NULL=未绑定',
    PRIMARY KEY (id),
...
CREATE TABLE product_review (
    ...
    reply_content VARCHAR(500) DEFAULT NULL COMMENT '商家回复正文，空=未回复',
    reply_at DATETIME DEFAULT NULL COMMENT '回复时间',
    hidden TINYINT NOT NULL DEFAULT 0 COMMENT '1 前台不展示（违规隐藏，保留已评价状态）',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (id),
...
-- 会员日：**指定的具体日期**（年月日）当天消费积分翻倍，后台「会员日」菜单维护（可配多个日期）。
-- 原先「会员日 每月18号 双倍积分」只是公告里的一句话、后端根本没实现（假承诺）。
-- ⚠️ 公告栏那条「会员日…」**不由系统改写** —— 那是运营文案，由管理员自己维护。
-- 历史：09-22 初版做的是「每月几号」（day_of_month 按月循环），当天按用户要求改成具体日期。
